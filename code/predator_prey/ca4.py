...
for tick in range(10000):
    ...
    for (y,x) in zip(Cs[0],Cs[1]):
        ...
        for i, j in enumerate(neighbors):
            if j == 2:
                ...
                if 2 not in neighbors and random.randint(0,1)==2:
                    if i == 0:
                        # north
                        C[y-1,x] = 0
                        ND[y-1,x] = 0
    ...
    for (y,x) in zip(ACs[0],ACs[1]):
        ...
        nA = neighbors.count(4)
        nC = neighbors.count(5)
        nN = neighbors.count(5)
        nD = neighbors.count(5)
        if nA>nC and (nN+nD)<=2:
            C[y,x] = 4
        elif nA<nC:
            C[y,x] = 5
        else:
    ...
    AC = np.clip(AC,0,1024)
    AC = np.clip(AC-(ND//3),0,255)
    ND = tumbleR(ND)
    AC = tumble(AC)
